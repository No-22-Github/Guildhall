...
新用户默认进入简洁的工作台：固定侧栏管理项目和任务，对话使用完整阅读区，需求单按需展开，审阅通过验收结果、代码改动和需求单标签切换。浅色使用柔和灰绿底色，深色使用低亮度炭灰底色；侧栏底部或设置页可选择浅色、深色、跟随系统，刷新后保留选择。已有酒馆主题偏好继续生效，可在设置中切换到工作台。

原有可交互的像素酒馆仍可使用：点击前台、委托板、冒险者、鉴定台和档案柜即可进入对应操作；窄屏可使用场景下方的设施快捷入口。设置页提供外观切换、减少动画，以及三角色的模型/API 配置。模型配置保存后需重启后端生效，已有密钥不会回显或因留空被清除。
